...
def test_profile_page(client, auth):
    ...
    assert b"href='/add_article" in response.data
...
def test_create(client, auth, app):
    auth.login()
    assert client.get('/add_article').status_code == 200
    client.post('/add_article', data={'title': 'created', 'bookTitle':'Dummy Book', 'file': 'https://dummyimage.com/150', 'body': 'Sample Body'})
